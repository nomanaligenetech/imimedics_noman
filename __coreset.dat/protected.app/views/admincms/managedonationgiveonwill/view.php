...
if ( $table_record -> num_rows() > 0 )
{
            ...
            foreach ( $table_record -> result_array() as $key => $row )
            {
                ...
                ?>
                    <tr>
                        ...
                        <td><?php echo $row["column_first_text"];?></td>
                        <td><?php echo $row['column_two_text'];?></td>
                        <td><?php echo $row["column_three_text"] ;?></td>
                        <td><?php echo $row["donation_way_to_give_address"] ;?></td>
                        <td><?php echo DropdownHelper::cmsmenubelongsto_dropdown(false, $row["belongsto"]);?></td>
                        <td>
                        <a href="<?php echo site_url( $_directory . "controls/edit/" . $row["id"]);?>" data-operationid="managedonationgiveonwilledit">
    ...
    <div class="crud_controls">
      <?php echo form_input( array("name" => "options", "type" => "hidden", "value" => "delete" ) )?>
        <a href="<?php echo site_url( $_directory . "controls/add" );?>" data-operationid="managedonationgiveonwilladd">
        	<input data-operation="add" type="button" class="btn btn-warning btn-flat submit_btn_form" value="<?php echo lang_line("text_add");?>"  />
        </a>
        <input data-operation="delete" data-operationid="managedonationgiveonwilldelete" type="button" class="btn btn-danger btn-flat submit_btn_form" value="<?php echo lang_line("text_delete");?>" />
       <!--  <a href="javascript:;">
        	<input data-operation="ajax_update_sorting" data-operationid="managedonationgiveonwillupdatesort" type="button" class="btn btn-primary btn-flat submit_btn_form" value="<?php echo lang_line("text_update_sort");?>"  />
        </a> -->
    </div>
</form>
