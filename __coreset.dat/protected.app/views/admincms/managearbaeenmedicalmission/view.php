    ...
    <table id="tbl_records" class="table table-bordered table-striped arbaeen-medical-mission-table-view">
        ...
        <tbody>
            ...
            foreach ($table_record  as $key => $row) {
                if (isset($row['id'])) {
                    $createDate = new DateTime($row["date_added"]);
                    ...
                    <tr class="<?php echo $row["status"] ?>">
                        ...
                        <td><?php echo (strtotime($strip) >= strtotime("2023-05-29")) ? $this->encrption->decrypt($row["phone_number"]) : $row["phone_number"]; ?></td>
                        <td><?php echo $row["date_added"]; ?></td>
                        <td><?php echo $row['membership_details_package_name']; ?></td>
                        <td><a href="<?php echo site_url() . 'assets/files/arbaeen-mission/' . $row['cv_resume']; ?>">Download</a></td>
                        ...
                        case 'assigntointerviewer':
                            ?>
                            <input type="button" class="btn btn-success btn-xs" value="Assigned To Interviewer" />
                            <?php
                            break;

                        case 'scheduleinterview':
                            ?>
                            <input type="button" class="btn btn-success btn-xs" value="Interview Scheduled" />
                            <?php
                            break;

                        case 'interviewcallrejected':
                            ?>
                            <input type="button" class="btn btn-danger btn-xs" value="Interview Call Rejected" />
                            <?php
                            break;
                        case 'recommended':
                            ?>
                            <input type="button" class="btn btn-success btn-xs" value="Recommended" />
                            <?php
                            break;

                        case 'not-recommended':
                            ?>
                            <input type="button" class="btn btn-danger btn-xs" value="Not Recommended" />
                            <?php
                            break;
                        case 'stage3':
                            ?>
                            <input type="button" class="btn btn-primary btn-xs" value="Stage3" />
                            <?php
                            break;
                    ...
                    ?>
                        <td>
                            ...
                            <?php } ?>
                            <?php if ($is_interviewer) { ?>
                                <?php if ($row['is_interview_call_accepted']) { ?>
                                ...
                                <?php } ?>
                                <?php if ($row['status'] == "assigntointerviewer" && !$row['is_interview_call_accepted']) { ?>
                                    <a href="<?php echo site_url($_directory . "controls/acceptinterviewcall/" . $row["id"]); ?>" data-operationid="managearbaeenmedicalmissionsacceptinterviewcall">
                                        <input type="button" class="btn btn-success btn-xs" value="Accept Interview Call" />
                                    </a>
                                    <a href="<?php echo site_url($_directory . "controls/rejectinterviewcall/" . $row["id"]); ?>" data-operationid="managearbaeenmedicalmissionsrejectinterviewcall">
                                        <input type="button" class="btn btn-danger btn-xs" value="Reject Interview Call" />
                                    </a>
                                <?php } ?>
                                <?php if ($row['status'] == "scheduleinterview") { ?>
                                    <a href="<?php echo site_url($_directory . "controls/interviewnotes/" . $row["id"]); ?>" data-operationid="managearbaeenmedicalmissioninterviewnotes">
                                        <input type="button" class="btn btn-info btn-xs" value="Update Interview Notes" />
                                    </a>
                                <?php } ?>
                            <?php } ?>
                            <?php if ($row['status'] == "recommended" || $row['status'] == "not-recommended") { ?>
                                <a href="<?php echo site_url($_directory . "controls/stage3/" . $row["id"]); ?>" data-operationid="managearbaeenmedicalmissionstage3">
                                    <input type="button" class="btn btn-primary btn-xs" value="Send Stage 3" />
                                </a>
                            <?php } ?>
                        </td>
                    </tr>
