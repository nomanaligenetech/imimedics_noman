            ...
            {				
                ...
                ?>
                    <tr>
						...
						if ( $row["parentid"] == 0)
						{
							echo ' <strong>('. $this->queries->fetch_records("conference_registration_screen_three", 
																			 " AND parentid = '". $row["id"] ."' AND screen_one_detail_id IN (SELECT id FROM `tb_conference_registration_screen_one_family_details` WHERE parentid IN (SELECT id FROM `tb_conference_registration_screen_one` WHERE conferenceregistrationid = tb_conference_registration_screen_three.conferenceregistrationid)) ")->num_rows() . ')</strong>';
						}
						?>
                        
                        </td>
                        <td><?php echo generate_participant_UID($row["id"]);?></td>
                        ...
                        <td><?php echo $row['VIEW_conference_master_date_added'];?></td>
                        ...
                        <td>
                        	...
                        	<?php echo $this->load->view("admincms/manageconferenceregistration/10/view_registration_prices", array('fetch_records_for_view_ROW' => $row)); ?>
