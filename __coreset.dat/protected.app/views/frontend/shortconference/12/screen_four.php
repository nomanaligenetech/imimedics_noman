...
<table cellpadding="2" cellspacing="5" width="100%" class="semiform tableform" >
    <tbody>
        ...
        <tr>
            <td colspan="2">
            	<?php
				$tmp_instructions			= $this->mixed_queries->fetch_records(	'short_conference_content_with_menu', 
																					" 	AND m.slug = 'conference-nine-step-four-instructions' 
																						AND m.conferenceid = '". $conference->row()->id ."' ");
																						
				if ( $tmp_instructions -> num_rows() > 0 )
				{
					echo $this->functions->content_shortcodes( $tmp_instructions->row()->content );
				}																		
				?>  
		...
		?>
			...
			{
				$is_completed							= $this->queries->fetch_records('short_conference_registration_screen_three', 
																						" AND parentid  = '". $conferenceregistration_screenthree->row("id") ."' 
																						  AND screen_one_detail_id = '". $f['id'] ."' ");
																						  
				
				$bg_active									= FALSE;  
				if ( $screen_one_detail_id == $f['id'] )
				{
					$bg_active								= "background-color:lightgray; padding:10px;";
				}
				
			?>
                <tr>
                    <td colspan="2" style=" <?php echo $bg_active;?>">
                    ...
                    <a style="text-decoration:none;" href="<?php echo site_url('shortconference/'. $conference->row("slug") .'/registration/screen/4/' . $f['id']);?>">
...
else
{
    ...
    <div class="flt-rgt">
        ...
        <a style="text-decoration:none;" href="<?php echo site_url( "shortconference/" . $conference->row("slug") . "/registration/screen/5" );?>">
