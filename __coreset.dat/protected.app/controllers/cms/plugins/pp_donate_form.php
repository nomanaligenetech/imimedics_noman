...
class PP_Donate_Form extends C_frontend {
...
//	    ~r(debug_backtrace());
		...
		PP_Donate_Form::extend_data( $data );
	...
	{	
		redirect(site_url().'page/donate');	
		if ( in_array("payment_success", $ci->uri->segments ) )
		{
			PP_Donate_Form::payment_success( $data, $ci );
		}
		...
		{
			PP_Donate_Form::payment_success_honor( $data, $ci );
		}
		...
		{
			PP_Donate_Form::payment_cancel( $data, $ci );
		}
		else
		{
			...
			if($ci->input->post("btn_donate_form") == false && $data['_pageview'] != "global/_blank_page.php"){
				$insertData = array(
					'ip_address' => $_SERVER["REMOTE_ADDR"],
			...
			PP_Donate_Form::_create_fields_for_form(false, $data);
			...
			{
				PP_Donate_Form::_create_fields_for_form(true, $data, $edit_details->row_array() );	
			...
			return $ci->load->view( "frontend/cms/page_plugins/pp_donate_form", $data, TRUE );
		}
	...
	static public function index( &$data = array(), $ci )
	{
		...
		if($ci->input->post("btn_donate_form")=="Send"){
			...
			$ci->form_validation->set_rules('homecountry', lang_line('text_country'), 'required');
			...
			if ($ci->form_validation->run() == FALSE)
			{
				...
				$tmp_countries			= $ci->imiconf_queries->fetch_records_imiconf("countries", " AND id = " . $ci->input->post("homecountry"))->result()[0];
				...
				$insertData					= array(
					...
					"home_country"	=> $ci->input->post("homecountry"),
					"user_id"		=> $ci->functions->_user_logged_in_details( "id" ),
					"date_added"	=> date("Y-m-d H:i:s"),
				);

				if($ci->input->post('homecountry') == 38){
	...
	static public function form_projects_accordion($name = '', $options = array(), $selected = array(), $extra = '', $ci = null)
	{
		if ( ! is_array($selected))
		{
			$selected = array($selected);
		...
		$form  = '<div class="projects-container">'; // name="'.$name.'"'.$extra.$multiple.">\n";
		...
		usort($options, PP_Donate_Form::build_sorter(5));
		...
		{
			$key = (string) $key;
			$sel = (in_array($key, $selected)) ? ' active' : '';
			$display_value	= (string) $val[0];

			if(count($val) > 4){
				$active=$display_list='';
				$form .= '<div class="cause-wrapper">';
					$sub_group = array_diff_key($val, array_flip([0,1,2,3]));
					unset($sub_group[5]);
					usort($sub_group, PP_Donate_Form::build_sorter(5));
					$class = strtolower(str_replace(' ', '', $display_value));
					$class = str_replace('&', '', $class);

					$array_keys  = array_keys($sub_group);
					$matched_arr = array_intersect($array_keys,$selected);
					
					if( !empty($matched_arr) ){
						$active = ' active';
						$display_list = "' style='display:block'";
					}

					if( is_countryCheck(FALSE,FALSE,TRUE) == 'canada' ){
						$display_value = str_replace("IMI", "IMI Canada", str_replace("IMI Canada", "IMI", $display_value));
					}

					$form .= '<a href="javascript:;" class="d_accordion '.$class.$active.'">'.$display_value.'</a>';
					$form .= '<div class="panel"'.$display_list.'>';
					foreach ( $sub_group as $k => $row )
					{
						if(is_array($row)){
							$sel = (in_array($k, $selected)) ? ' active' : '';
							$info = $row[1].$sel === 'camp' ? '<a href="'.site_url("page/donation-campaigns/$row[4]").'"><?xml version="1.0"?><svg fill="#000000" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24" width="15px" height="15px"><path d="M 12 2 C 6.4889971 2 2 6.4889971 2 12 C 2 17.511003 6.4889971 22 12 22 C 17.511003 22 22 17.511003 22 12 C 22 6.4889971 17.511003 2 12 2 z M 12 4 C 16.430123 4 20 7.5698774 20 12 C 20 16.430123 16.430123 20 12 20 C 7.5698774 20 4 16.430123 4 12 C 4 7.5698774 7.5698774 4 12 4 z M 11 7 L 11 9 L 13 9 L 13 7 L 11 7 z M 11 11 L 11 17 L 13 17 L 13 11 L 11 11 z"/></svg></a>' : '';
	
							$form .= '<div class="d_option '.$row[1].$sel.'" data-display="'.$row[0].'" data-value="'.$row[6].'">'.$row[0];
							$form .= $info;
							$form .= '</div>';
							if(isset( $_POST['dp_donor_comment'] ) && $_POST['donation_projects'] == $row[6] ){
								$form .= '<input type="text" name="dp_donor_comment" id="dp_user_input" class="dp_user_input" value="'.$_POST['dp_donor_comment'].'" />';
							}

							// $form .= '<input type="text" name="dp_user_input_'.$k.'" id="dp_user_input_'.$k.'" style="display:none;" />';
						}
					}
					$form .= "</div>";
				$form .= "</div>";
				...
				// $form .= 'testing';
				// $form .= '<option value="'.$key.'"'.$sel.' class="'.$val[1].'" type="'.$val[2].'">'.$display_value."</option>\n";
			}
		}
