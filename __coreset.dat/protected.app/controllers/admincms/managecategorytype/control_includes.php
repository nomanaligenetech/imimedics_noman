...
class Controls_Include extends C_admincms {
	...
	public function save_detail( $THIS, $LOAD_VIEW, &$data, $created_by_user = NULL, $created_by_admin = NULL )
	{
		...
		$this->form_validation->set_rules("cat_type_name", "Category Type", "trim|required");
	//	$this->form_validation->set_rules("cat_name", "Category Name", "trim|required");
		...
		else
		{
			 ...
			 $saveData											= array("cat_type_name"	=> $this->input->post('cat_type_name'),
																							);
	...
	if ($THIS->input->post('options') == "edit")
			{
				...
				$THIS->queries->SaveDeleteTables($saveData, 'e', "tb_category_type", 'id');  
			}
			else
			{
				$THIS->queries->SaveDeleteTables($saveData, 's', "tb_category_type", 'id');  		
	...
	public function delete( $THIS, $id )
	{
		...
		{
			$saveData['id']									= $result;	
			$THIS->queries->SaveDeleteTables($saveData, 'd', "tb_category_type", 'id') ;
		}
	...
	public function _create_fields_for_form( $return_array = false, &$data, $db_data = array() )
	{
	    ...
	    $empty_inputs				= array( "id","cat_type_name","options");
		
		$filled_inputs				= array( "id","cat_type_name","options");
