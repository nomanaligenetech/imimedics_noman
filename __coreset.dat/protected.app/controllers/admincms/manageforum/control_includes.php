...
class Controls_Include extends C_admincms {
	...
	public function save_detail( $THIS, $LOAD_VIEW, &$data, $created_by_user = NULL, $created_by_admin = NULL )
	{
		...
		$this->form_validation->set_rules("category", "Category Type", "trim|required");
		$this->form_validation->set_rules("name", "Forum Name", "trim|required");
	...
	if ($THIS->input->post('options') == "edit")
			{
				...
				$THIS->queries->SaveDeleteTables($saveData, 'e', "tb_forum", 'id');  
			}
			else
			{
				$THIS->queries->SaveDeleteTables($saveData, 's', "tb_forum", 'id');  		
	...
	public function delete( $THIS, $id )
	{
		...
		{
			$saveData['id']									= $result;	
			$THIS->queries->SaveDeleteTables($saveData, 'd', "tb_forum", 'id') ;
		}
	...
	public function _create_fields_for_form( $return_array = false, &$data, $db_data = array() )
	{
	    ...
	    $empty_inputs				= array( "id","name","description","slug","date","created_by_admin","options");
		
		$filled_inputs				= array( "id","name","description","slug","date","created_by_admin","options");
