...
class Controls extends C_admincms {
	...
	public function __construct()
	{
		...
		$this->data["_heading"]										= 'Manage Languages';
    ...
    {
        $tmp["tr_heading"] = array('Language', 'Code', 'RTL/LTR');
    ...
    public function view()
    {
		...
		$data["table_record"] = $this->queries->fetch_records("content_languages", " ORDER BY id DESC ");
	...
	public function edit($edit_id)
	{
		...
		$edit_details = $this->queries->fetch_records("content_languages", " AND id = '$edit_id'");
	...
	public function _create_fields_for_form( $return_array = false, &$data, $db_data = array() )
	{
		...
		$empty_inputs				= array("id", "name", "code", "direction", "options", "unique_formid" );
		
		$filled_inputs				= array("id", "name", "code", "direction", "options", "unique_formid" );
	...
	public function delete( $id )
	{
		...
		{
			$saveData['id']									= $result;	
			$this->queries->SaveDeleteTables($saveData, 'd', "tb_content_languages", 'id') ;
		}
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('code', 'Code', 'trim|required|max_length[2]');
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->input->post('options') == "edit")
			{
				...
				$this->queries->SaveDeleteTables($saveData, 'e', "tb_content_languages", 'id');  
			}
			else
			{
				$this->queries->SaveDeleteTables($saveData, 's', "tb_content_languages", 'id');  		
