...
class Controls extends C_admincms {
	...
	public function __construct()
	{
		...
		$this->data["_heading"]										= 'Manage Testimonials';
	...
	{
		$tmp["tr_heading"]											= array('Name', 'Email', 'Testimonial', 'Sort', 'Status');
	...
	public function view( $is_ajax = 0 )
	{
		...
		$data["table_record"]										= $this->queries->fetch_records("testimonials", " ORDER BY id desc ");
	...
	public function ajax_update_sorting( $id )
	{
		...
		foreach ($id	as $key	=> $result)
		{
			...
			$this->queries->SaveDeleteTables($saveData, 'e', "tb_testimonials", 'id') ;
		}
	...
	public function save ()
	{
		...
		if ($this->input->post("menu")) {
			$_POST['menu'] = implode(",",$this->input->post("menu"));
		} else {
			$_POST['menu'] = "";
		}
		
		if ($this->input->post("events")) {
			$_POST['events'] = implode(",",$this->input->post("events"));
		} else {
			$_POST['events'] = "";
		}
		...
		else
		{
			...
			{
				$this->queries->SaveDeleteTables($saveData, 's', "tb_testimonials", 'id');  		
	...
	public function _create_fields_for_form( $return_array = false, &$data, $db_data = array() )
	{
		...
		$empty_inputs				= array( "id", "username", "menu", "events", "email", "testimonial", "status", "options", "unique_formid" );
		
		$filled_inputs				= array( "id", "username", "menu", "events", "email", "testimonial", "status", "options", "unique_formid" );
	...
	public function edit( $edit_id )
	{
		...
		$edit_details												= $this->queries->fetch_records("testimonials", " AND id = '$edit_id' ");
	...
	public function delete( $id )
	{
		...
		{
			$saveData['id']									= $result;	
			$this->queries->SaveDeleteTables($saveData, 'd', "tb_testimonials", 'id') ;
		}
