...
class Controls extends C_admincms {
	...
	public function __construct()
	{
		...
		$this->data["_heading"]										= 'Manage Members';
...
EOT;
		...
		SELECT
			...
			IF(
				membership_package_price IS NULL or membership_package_price = ' ',
				um_p.price,
...
EOT;
    ...
    {
        $tmp["tr_heading"] = array('Name', 'Last Name', 'Email', 'Password','Address','Country', 'Registration Site', 'Membership Package', 'Membership Price', 'Paid Membership Status', 'Member Status', 'Member Blocking Status', 'Date Added', 'Expiry Date' ,'Tax Receipt');
    ...
    public function view($is_ajax = 0)
    {
        ...
        if ($is_ajax) {
			...
			$tmp_columns[] = array(
                'db' => 'home_full_address',
                "where" => "home_full_address", #takes 1 field but i also have to pass ID - this scenerio will return correct output.
                'dt' => 5,
            );
			$tmp_columns[] = array(
                'db' => 'home_country_name',
                "where" => "home_country_name", #takes 1 field but i also have to pass ID - this scenerio will return correct output.
                'dt' => 6,
            );
			...
			$tmp_columns[] = array(
                'db' => 'package_price',
                "where" => "package_price", #takes 1 field but i also have to pass ID - this scenerio will return correct output.
                'dt' => 9,
            );
            $tmp_columns[] = array(
            ...
            $tmp_columns[] = array(
                'db' => 'member_expiry',
                "where" => "member_expiry", #takes 1 field but i also have to pass ID - this scenerio will return correct output.
                'dt' => 14,
            );
           
			$tmp_columns[] = array(
                'db' => 'um_id',
                "where" => "um_id", #takes 1 field but i also have to pass ID - this scenerio will return correct output.
                'dt' => 15,
				'formatter' => function ($d, $row, $CI) {
					$TMP_receipt_table	= $this->db->query("SELECT receipt_number, receipt_prefix FROM tb_payment_receipts WHERE table_name = 'tb_user_memberships' AND table_id_value = '". $row['um_id'] ."' ");
					return (
                        (
							($TMP_receipt_table->num_rows() > 0) ? 
							'<a href="'. site_url($this->data['_directory'] . "controls/receipt/" . $row['um_id']) .'" data-user-id="' .
                            $row['id'] .
                        '" class="btn btn-success btn-sm" data-operationid="managememberstaxreceipt">Download</a>' : 'N/A'
                        )
                    );
                },
            );

            $columns = SSP::append_columns($tmp_columns, array("id", "id"), array(0,16));
			$primaryKey = 'id';
			...
			// THIS CHECK IS FOR CANADA ONLY 38 IS HOME_COUNTRY ID OF CANADA
			$country_where = '';
			$get_belongsto 		= $this->functions->_admincms_logged_in_details( "belongs_country" );
			if($get_belongsto == "3"){
				$country_where = ' and ( home_country = 38 )';
			}

			
			$where = ' and ( u_ispaid = 1 or is_paid_membership_approved = 1 )';
			$table = "( SELECT * FROM users_view where 1=1 $where $country_where ) t0 where 1=1";
	...
	public function receipt( $ms_id ){
		...
		if( $TMP_table->num_rows() > 0 )
		{
			...
			$TMP_receipt_table	= $this->db->query("SELECT receipt_number, receipt_prefix FROM tb_payment_receipts WHERE table_name = 'tb_user_memberships' AND table_id_value = '". $ms_id ."' ");

			$pdfData = array(
				"name"			=> $TMP_table->row()->first_name,
				"address"		=> $TMP_table->row()->city . $_homestate,
			...
			$file_name = 'tax-receipt-' . $ms_id . '.pdf';
		...
		SELECT * FROM (
			(
				SELECT
					...
					null as 'Age',
					registration_site
				FROM ( SELECT * FROM users_view where ( u_ispaid = 1 or is_paid_membership_approved = 1 ) $country_where ) t0 order by case when imi_id regexp '^[0-9]' then 1 when imi_id regexp '^[a-zA-Z]' then 2 when imi_id = '' or imi_id is null then 3 end, imi_id * 1,CAST(imi_id AS UNSIGNED)
			)
			union all
			(
				SELECT
