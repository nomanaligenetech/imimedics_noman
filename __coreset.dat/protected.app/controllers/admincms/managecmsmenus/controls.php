...
class Controls extends C_admincms {
	...
	{
		$tmp["tr_heading"]											= array( 'Name', 'Sub-Heading', 'Slug', 'Position', 'Type', 'Int/Can', 'Sort', 'Last Updated');
	...
	public function save ()
	{
		...
		$this->rules_for_array_content("subheading", "Sub-Heading", "trim");
		...
		// echo "<pre>";
		// print_r ($this->input->post("name[EN]"));
		// echo "</pre>";
		// exit;
		...
		if( $this->form_validation->run() == FALSE )
		{
			...
			$tmp_ajax_output1									= (new AjaxMethods)->cmsmenu_with_position( set_value("positionid"), FALSE );
		...
		else
		{
			...
			$saveData											= array("parentid"						=> $parentid,
																		...
																		"subheading"					=> $subheading[DEFAULT_LANG_CODE],
	...
	private function rules_for_array_content($field = 'content', $label, $rule = "trim|required")
	{
	...
	private function _save_lang_content($ref_id, $action){
		...
		$cmsmenu = $this->queries->fetch_records("cmsmenu_languages", " AND cmsmenu_id = '$ref_id' ")->result_array();

		foreach ($this->data['content_languages'] as $lang_key => $lang) {
			$saveData = [
				'name' => $name[$lang['code']],
				'subheading' => $subheading[$lang['code']],
				'cmsmenu_id' => $ref_id,
				'content_languages_id' => $lang['id']
			];
			
			// if(!$cmsmenu && !in_array($lang['id'], array_column($cmsmenu,'content_languages_id'))){
			if(!$cmsmenu || ($cmsmenu && !in_array($lang['id'], array_column($cmsmenu,'content_languages_id')))){
				$col = 'id';
				$action = 's';
			}else{
				$col = 'content_languages_id|cmsmenu_id';
				$action = 'e';
			}
			
			$this->queries->SaveDeleteTables($saveData, $action, "tb_cmsmenu_languages", $col);
		}
	}
	...
	public function _create_fields_for_form( $return_array = false, &$data, $db_data = array() )
	{
		...
		$empty_inputs				= array( "id", "show_title", "show_icon_with_title", "slug", "target", "typeid", "photo_image", "photo_image_icon", "positionid", "status", "parentid", "belongsto", "sort", "options", "unique_formid" );
		
		$filled_inputs				= array( "id", "show_title", "show_icon_with_title", "slug", "target", "typeid", "photo_image", "photo_image_icon", "positionid", "status", "parentid", "belongsto", "sort", "options", "unique_formid" );
	...
	public function _create_lang_fields_for_form( $return_array = false, &$data, $loop_object = array(), $db_data = array() )
	{
		...
		$empty_inputs				= array("name", "subheading");
		$filled_inputs				= array("name", "subheading");
	...
	public function edit( $edit_id )
	{
		...
		$edit_lang_details									= $this->queries->fetch_records("cmsmenu_languages", " AND cmsmenu_id = '$edit_id' ")->result_array();
		...
		$tmp_ajax_output1									= (new AjaxMethods())->cmsmenu_with_position( $edit_details["positionid"], FALSE );
		if ( $tmp_ajax_output1["_CSS_show_messages"] != "error" )
		{
			$data['ajax_output1']							= $tmp_ajax_output1["_TEXT_show_messages"];
		}
	...
	public function delete( $id )
	{
		...
		foreach ($id	as $key	=> $result)
		{
			...
			$this->queries->SaveDeleteTables(['cmsmenu_id'=>$result], 'd', "tb_cmsmenu_languages", 'cmsmenu_id') ;
		}
