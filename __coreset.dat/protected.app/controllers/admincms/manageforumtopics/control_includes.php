...
class Controls_Include extends C_admincms {
	...
	public function save_detail( $THIS, $LOAD_VIEW, &$data, $created_by_user = NULL, $created_by_admin = NULL )
	{
		...
		$this->form_validation->set_rules("forum", "Forum Type", "trim|required");
		$this->form_validation->set_rules("name", "Forum Topic", "trim|required");
...
*/		
	...
	public function _create_fields_for_form( $return_array = false, &$data, $db_data = array() )
	{
	    ...
	    $empty_inputs				= array( "id","frmid","name","description","slug","date","created_by_admin","options");
		
		$filled_inputs				= array( "id","frmid","name","description","slug","date","created_by_admin","options");
