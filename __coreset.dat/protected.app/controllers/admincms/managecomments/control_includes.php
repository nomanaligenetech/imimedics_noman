...
class Controls_Include extends C_admincms {
	...
	public function save_detail( $THIS, $LOAD_VIEW, &$data, $created_by_user = NULL, $created_by_admin = NULL )
	{
	    ...
	    //$userid=$this->functions->_admincms_logged_in_details('id'); 
...
*/		
		...
		else
		{
			 ...
			 $saveData											= array("comment"	=> $this->input->post('comment'),
																		);
	...
	if ($THIS->input->post('options') == "edit")
			{
				...
				$THIS->queries->SaveDeleteTables($saveData, 'e', "tb_comments", 'id');  
			}
			else
			{
	...
	public function delete( $THIS, $id )
	{
		...
		foreach ($id	as $key	=> $result)
		{
			$row											= $THIS->queries->fetch_records("comments", " AND id = '$result' ")->row();
		...
		{
			$saveData['id']									= $result;	
			$THIS->queries->SaveDeleteTables($saveData, 'd', "tb_comments", 'id') ;
		}
	...
	public function _create_fields_for_form( $return_array = false, &$data, $db_data = array() )
	{
	    ...
	    $empty_inputs				= array("id","post_id","comment","parentid","user_id","options");
		
		$filled_inputs				= array("id","post_id","comment","parentid","user_id","options");
