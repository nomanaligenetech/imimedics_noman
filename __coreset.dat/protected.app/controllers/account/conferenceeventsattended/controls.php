...
class Controls extends C_frontend {
	...
	public function __construct()
	{
		...
		$this->data["_heading"]										= lang_line('text_confeventsattended');
	...
	{		
		...
		#
		$data['files_details_1']									= $this->imiconf_queries->fetch_records_imiconf("conference_registration_screen_three", 
		...
		$data['files_details_1']									= $this->imiconf_queries->fetch_records_imiconf("conference_registration_excel", 
																											" AND user_id = '". $this->functions->_user_logged_in_details( "id" ) ."' ");	
		
		$data['files_details_2']									= $this->queries->fetch_records("short_conference_registration_screen_three", 
																											" AND conferenceregistrationid IN ( SELECT id FROM tb_short_conference_registration_master WHERE userid = '". $this->functions->_user_logged_in_details( "id" ) ."' ) ");
		#$this->load->library("Encrption");
		#echo $this->encrption->decrypt("VwAmSaPM/mb2bAtnfuuS0nL1H0alixEfm5Y="); //Woodbridge001
		#die;
		if ( $data['files_details_1'] -> num_rows() <= 0 )
		{
