...
class Controls extends C_frontend {
	...
	public function __construct()
	{
		...
		$this->data["_heading"]										= 'IMI Volunteer Form';
	...
	public function _create_fields_for_form( $return_array = false, &$data, $db_data = array() )
	{
		...
		$empty_inputs				= array( "id", "name", "date_of_birth", "email", "phone", "city", "state", "country", "qualification", "area_of_interest","user_id", "date_added", "type", "options", "unique_formid" );
		
		
		
		$filled_inputs				= array( "id", "name", "date_of_birth", "email", "phone", "city", "state", "country", "qualification", "area_of_interest","user_id", "date_added", "type", "options", "unique_formid" );
	...
	{		
		...
		$edit_details 												= $this->queries->fetch_records("volunteer_form", " AND user_id = '".$this->functions->_user_logged_in_details( "id" )."' ORDER BY `id` DESC");
		...
		else
		{
			...
			$this->queries->SaveDeleteTables($insertData, 'e', "tb_volunteer_form", 'user_id'); 
			
			
			
			$data['_messageBundle']									= $this->_messageBundle( 'success' , 
																							 "Your volunteer form updated.", 
