...
EOQ
	...
	*/
	...
	function send_tax_receipt($df_id, $ci, $is_recurring=false, $platform = '', $_event_id){

		// pdfwork start			
		$TMP_receipt	= $ci->db->query("SELECT 
		df.*, 
		dcc.comment as comments, 
		dpp.name as dpdesc, 
	  ...
	  FROM 
		tb_donation_form df 
		LEFT JOIN `tb_dc_comments` dcc ON df.id = dcc.df_id 
		...
		if( $TMP_receipt->num_rows() > 0 && ($TMP_receipt->row()->belongs_country == '2' || $TMP_receipt->row()->belongs_country == '3' || $TMP_receipt->row()->belongs_country == '4') )
		{
			$canada_paypal_receiver_id = 'RWBBGATGJXAKE';
			$paypal_post = !empty($TMP_receipt->row()->paypal_post) ? unserialize($TMP_receipt->row()->paypal_post) : '';
			$receiver_id = isset($paypal_post['receiver_id']) ? $paypal_post['receiver_id'] : '';

			$belongs_to  = $TMP_receipt->row()->belongs_country; // belongs country
			// canada
			if($belongs_to == '3' && $TMP_receipt->row()->payment_mode == 'paypal' && $receiver_id == $canada_paypal_receiver_id){
			...
			elseif($belongs_to == '4'){
				if($platform == 'events'){
					$event_details	= $ci->db->query("SELECT package_amount,package_title FROM `tb_event_registrations` er INNER JOIN tb_event_packages ep ON er.package_id = ep.id  WHERE donation_form_id = {$TMP_receipt->row()->id}");
					$package_amount = $event_details->row()->package_amount;
					$package_title  = $event_details->row()->package_title;
					$receipt_pdf 	= "receipts/global-events-international.php";
					$email_subject	= "Your Tax Receipt for the Event Registration";
					$event_email_heading = 'Event Registration';
					$receipt_email 	     = "email/frontend/event_receipt_email.php";
					
					if($_event_id == 163){
						$receipt_email 	= "email/frontend/dinner-auction-umc.php";
						$_POST["donation_url"]   = "https://medicsinternational.org/page/donation-campaigns/university-medical-complex---karachi-";
						$_POST["email_start"]   = "Thank you for registering for our upcoming Gala Dinner for the University Medical Complex, Karachi, Pakistan. Your receipt for tickets purchased (as well as any donation made) is attached.";
					}
				}
				else{
			...
			$receipt_prefix = 'MI';
			$email_bcc		= array("imifinance786@gmail.com" ,"sakinarizviimi@gmail.com", "neelam.raheel@genetechsolutions.com", "taha.fatima@genetechsolutions.com");
			} else {
				if($platform == 'events'){
					$event_details	= $ci->db->query("SELECT package_amount,package_title FROM `tb_event_registrations` er INNER JOIN tb_event_packages ep ON er.package_id = ep.id  WHERE donation_form_id = {$TMP_receipt->row()->id}");
					...
					if($_event_id == 163){
						$receipt_email 	         = "email/frontend/dinner-auction-umc.php";
						$_POST["donation_url"]   = "https://imamiamedics.com/page/donation-campaigns/university-medical-complex---karachi";
						$_POST["email_start"]   = "Thank you for registering for our upcoming “Dinner and Auction” for the University Medical Complex, Karachi, Pakistan.  Your receipt for the tickets purchased is attached.";
					}else{
						$receipt_email 	         = "email/frontend/event_receipt_email.php";
					}

					$event_email_heading = 'Event Registration';
					// Make the following dynamic for future
					$_POST['event_details']['name'] = 'IMI Mega Fundraiser for The Dream Project';
				}else{
			...
			$pdfData = array(
				...
				"home_city"				=> $TMP_receipt->row()->home_city,
				"home_state_name"		=> $TMP_receipt->row()->home_state_name,
				"home_zipcode"			=> $TMP_receipt->row()->home_zipcode,
				"country_name"			=> $TMP_receipt->row()->country_name,
			...
			$email_template		= array("email_to"					=> $TMP_receipt->row()->email,
										"email_heading"				=> isset($event_email_heading) ? $event_email_heading : "Donation Tax Receipt",
	...
	function send_tax_receipt_honor($df_id, $ci){
	  ...
	  FROM 
		...
		$_POST['donation_details']['first_name'] = isset($TMP_receipt->row()->fromname) ? $TMP_receipt->row()->fromname : '';
		if( $TMP_receipt->num_rows() > 0 && ($TMP_receipt->row()->belongs_country == '2' || $TMP_receipt->row()->belongs_country == '3' || $TMP_receipt->row()->belongs_country == '4') )
		{
			$canada_paypal_receiver_id = 'RWBBGATGJXAKE';
			$paypal_post = !empty($TMP_receipt->row()->paypal_post) ? unserialize($TMP_receipt->row()->paypal_post) : '';
			...
			if( count($get_max_receipt_no->row()) > 0 ){
				...
				$dataForReceipt['receipt_number']	= $max_receipt_num;
				$dataForReceipt['receipt_prefix']	= $receipt_prefix;
				$dataForReceipt['table_name']		= 'tb_give_honor_someone';
				$dataForReceipt['table_id_name']	= 'id';
				$dataForReceipt['table_id_value']	= $df_id;
				$dataForReceipt['created_at']	    = $current_date->format('Y-m-d H:i:s');
			...
			$pdfData = array(
				"name"					=> $TMP_receipt->row()->fromname,
				"address"				=> $TMP_receipt->row()->home_address,
				"home_city"				=> $TMP_receipt->row()->home_city,
				"home_state_name"		=> $TMP_receipt->row()->home_state_name,
				"home_zipcode"			=> $TMP_receipt->row()->home_zipcode,
				"country_name"			=> $TMP_receipt->row()->country_name,
				"email"					=> $TMP_receipt->row()->card_email,
			...
			$email_template		= array("email_to"					=> $TMP_receipt->row()->card_email,
	...
	function send_tax_receipt_shortconference($userid, $ci, $conference_slug){
		...
		if( $_POST['conferenceregistration_screenone']->num_rows() > 0 )
		{
			...
			$receipt_prefix = 'SC';
			$email_bcc		= array('Imifinance786@gmail.com','sakinarizviimi@gmail.com','imiwaiting@att.net','neelam.raheel@genetechsolutions.com','imihq@imamiamedics.com');
